...
class PartReplacementTracker:
    ...
    def extract_replacements(self, production_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Create replacement tracking table
        replacement_events = pd.DataFrame({
            ...
            'replacement_detected_ts': datetime.now().isoformat(),
            'notes': replacements['calculation_notes']
        })
        
        # Calculate part lifecycle metrics
        replacement_events['part_wafers_at_replacement'] = replacement_events['last_value_before_replacement']
        
        # Remove duplicates (same entity + date + part)
        before_dedup = len(replacement_events)
        replacement_events = replacement_events.drop_duplicates(
            subset=['ENTITY', 'replacement_date', 'part_counter_name'],
            keep='last'
        )
